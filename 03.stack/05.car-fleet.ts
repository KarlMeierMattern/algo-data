...
const carFleet = (target: number, pos: number[], speed: number[]) => {
  const fleetStack: number[] = [];

  const paired = pos
    .map((p, index) => ({ p, s: speed[index] }))
    .sort((a, b) => b.p - a.p);

  for (const { p, s } of paired) {
...
console.log(carFleet(10, [4, 1, 0, 7], [2, 2, 1, 1])); // output 3
